...
function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetch('/api/auth/me', { credentials: 'include' }), fetch('/api/auth/student/me', { credentials: 'include' })])
      .then(async ([adminResponse, studentResponse]) => {
        const studentResult = studentResponse.ok ? await studentResponse.json() : null;
        if (adminResponse.ok) setState((current) => ({ ...current, adminAuthenticated: true, mode: 'admin' }));
        if (studentResult?.authenticated) setState((current) => ({ ...current, studentAuthenticated: true, studentUser: studentResult.user }));
      })
      .catch(() => undefined)
...
function StudentShell({ children }: { children: ReactNode }) {
  ...
  if (!state.studentAuthenticated) return <StudentCommunityAuth onAuthenticated={(user) => updateState({ studentAuthenticated: true, studentUser: user })} />;
  return <div className="student-shell"><StudentTopbar /><main className="student-content">{children}</main><StudentBottomNav /></div>;
}

function StudentAuthShell({ children }: { children: ReactNode }) {
  return <div className="student-shell"><header className="topbar" style={{ padding: '.85rem 1rem' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><Link href="/" style={{ color: 'inherit', textDecoration: 'none' }}><LogoMark compact /></Link><InstallAppButton /></div></header><main className="student-content">{children}</main></div>;
}
...
function CheckInPage() {
  ...
  return (
    <StudentShell>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', marginTop: '2rem', color: 'hsl(var(--muted-foreground))', fontSize: '.67rem' }}><LockKeyhole size={13} /> Stored privately on this device. You control what is shared.</div>
    </StudentShell>
  );
}
...
function StudentCommunityAuth({ onAuthenticated }: { onAuthenticated: (user: StudentUser) => void }) {
  ...
  const [error, setError] = useState('');
  const interestOptions = ['Study', 'Music', 'Gaming', 'Art', 'Sports', 'Tech', 'Books', 'First year'];
  const switchMode = (nextRegistering: boolean) => {
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    setError('');
    const endpoint = registering ? '/api/auth/student/register' : '/api/auth/student/login';
    const body = registering ? { email, username, password, interests } : { email, password };
    try {
      ...
      setError('The auth service is unavailable. Start the API server and try again.');
    }
  };
  return <StudentAuthShell><div className="community-auth-page animate-rise"><div className="community-auth-copy"><div className="eyebrow">Welcome to Mindful Campus</div><h1 className="display">Your private campus space starts here.</h1><p className="muted">Create one account for check-ins, wellness tools, support, and pseudonymous community conversations. Your password is never stored in the browser.</p><div className="auth-trust"><div><ShieldCheck size={16} /><span>Username-first community identity</span></div><div><UsersRound size={16} /><span>Rooms shaped by students</span></div><div><LockKeyhole size={16} /><span>Secure session, remembered on this device</span></div></div></div><form className="card community-auth-form" onSubmit={submit}><div className="auth-tabs"><button type="button" className={registering ? 'active' : ''} onClick={() => switchMode(true)}>Create account</button><button type="button" className={!registering ? 'active' : ''} onClick={() => switchMode(false)}>Sign in</button></div>{registering && <label className="field-label">Username<input className="input-field" required value={username} onChange={(event) => setUsername(event.target.value)} placeholder="e.g. quietcomet" data-testid="input-student-username" /></label>}<label className="field-label">Email<input className="input-field" required type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" data-testid="input-student-email" /></label><label className="field-label">Password<input className="input-field" required minLength={8} type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="At least 8 characters" data-testid="input-student-password" /></label>{registering && <div><div className="eyebrow" style={{ marginBottom: '.5rem' }}>What are you into?</div><div className="interest-picks">{interestOptions.map((interest) => <button type="button" key={interest} className={interests.includes(interest) ? 'selected' : ''} onClick={() => setInterests((current) => current.includes(interest) ? current.filter((item) => item !== interest) : [...current, interest])}>{interest}</button>)}</div></div>}{error && <div className="form-error">{error}</div>}<button className="primary-button" type="submit" style={{ width: '100%', minHeight: '3rem' }}>{registering ? 'Enter Mindful Campus' : 'Sign in to Mindful Campus'} <ArrowRight size={15} /></button></form></div></StudentAuthShell>;
}
function InstallAppButton() {
  ...
  const install = async () => {
    if (!installPrompt) { notify('On iPhone: tap Share, then Add to Home Screen. On Android: open the browser menu and tap Install app.'); return; }
    await installPrompt.prompt();
    setInstallPrompt(null);
  };
  return <button className="install-app-button" onClick={install}><DownloadIcon /> Install on your phone</button>;
}

type BeforeInstallPromptEvent = Event & { prompt: () => Promise<void> };
function DownloadIcon() { return <span aria-hidden="true" style={{ fontSize: '1rem', lineHeight: 1 }}>↓</span>; }

/*
  return <StudentAuthShell><div className="community-auth-page animate-rise"><div className="community-auth-copy"><div className="eyebrow">Welcome to Mindful Campus</div><h1 className="display">Your private campus space starts here.</h1><p className="muted">Create one account for check-ins, wellness tools, support, and pseudonymous community conversations. Your password is never stored in the browser.</p><div className="auth-trust"><div><ShieldCheck size={16} /><span>Username-first community identity</span></div><div><UsersRound size={16} /><span>Rooms shaped by students</span></div><div><LockKeyhole size={16} /><span>Secure session, remembered on this device</span></div></div><InstallAppButton /></div><form className="card community-auth-form" onSubmit={submit}>
*/
function CommunityLivePage() {
  const { notify, updateState } = useApp(); const [user, setUser] = useState<StudentUser | null>(null); const [rooms, setRooms] = useState<LiveRoom[]>([]); const [selectedRoom, setSelectedRoom] = useState<LiveRoom | null>(null); const [selectedChannel, setSelectedChannel] = useState<LiveChannel | null>(null); const [messages, setMessages] = useState<LiveMessage[]>([]); const [draft, setDraft] = useState(''); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [sending, setSending] = useState(false);
