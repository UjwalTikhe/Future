...
function CommunityLivePage() {
  ...
  useEffect(() => {
    ...
    const refresh = window.setInterval(() => {
      fetch(`/api/community/rooms/${selectedRoom.slug}/messages`, { credentials: 'include' }).then((response) => response.ok ? response.json() : []).then(setMessages).catch(() => undefined);
    }, 5000);
  ...
  const createRoom = async () => {
    const name = window.prompt('Name your new community room');
    if (!name?.trim()) return;
    const description = window.prompt('What is this room for?') ?? '';
    if (!description.trim()) return;
    const response = await fetch('/api/community/rooms', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, description }) });
    const room = await response.json();
    if (!response.ok) { notify(room.message ?? 'Room could not be created.'); return; }
    setRooms((current) => [...current, room]);
    await chooseRoom(room);
    notify('Your community room is live');
  };
